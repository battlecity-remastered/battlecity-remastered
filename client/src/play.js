...
export const DIRECTION_STEPS = 32;
...
const DIRECTION_VECTORS = Array.from(
  { length: DIRECTION_STEPS },
  (_unused, direction) => {
...
const getDirectionVector = (direction) =>
  DIRECTION_VECTORS[normaliseDirection(direction)];
...
const applyHospitalHealing = (game) => {
  if (
    ...
    typeof game.buildingFactory.findBuildingAtTile !== "function"
  ) {
    return;
  }
  ...
  if (
    !Number.isFinite(game.player.health) ||
  ...
  const lastHeal = Number.isFinite(game.player.lastHospitalHealAt)
    ? game.player.lastHospitalHealAt
    : 0;
  ...
  game.player.health = Math.min(
    MAX_HEALTH,
    previousHealth + HOSPITAL_HEAL_AMOUNT,
  );
...
const isBlockingCollision = (collisionCode) =>
  BLOCKING_COLLISIONS.has(collisionCode);
...
const shouldUseJoystickTurn = (game) =>
  !!game?.controlPreferences?.joystickControlsEnabled;

const applyJoystickAutoTurn = (game) => {
  if (!shouldUseJoystickTurn(game) || !game?.player) {
    return;
  }
  const targetDirection = Number.isFinite(game.player.joystickTargetDirection)
    ? normaliseDirection(game.player.joystickTargetDirection)
    : null;
  if (targetDirection === null) {
    if (game.player.isTurning !== 0) {
      game.player.isTurning = 0;
    }
    return;
  }
  const current = normaliseDirection(game.player.direction);
  ...
  const delta = (targetDirection - current + DIRECTION_STEPS) % DIRECTION_STEPS;
  let turnDirection = 0;
  if (delta === 0) {
    turnDirection = 0;
  } else if (delta === DIRECTION_STEPS / 2) {
    turnDirection = game.player.joystickLastTurnDirection || 1;
  } else {
    turnDirection = delta < DIRECTION_STEPS / 2 ? 1 : -1;
  }
  game.player.isTurning = turnDirection;
  game.player.joystickLastTurnDirection = turnDirection;
};
...
var movePlayer = (game) => {
  ...
  const movementScale =
    game.player.isMoving * (game.timePassed * MOVEMENT_SPEED_PLAYER);

  const applyAxisMovement = (
    axis,
    velocityComponent,
    positiveEdge,
    negativeEdge,
  ...
  if (
    ...
    console.log(
      `Player position: (${game.player.offset.x.toFixed(2)}, ${game.player.offset.y.toFixed(2)})`,
    );
  }
};
...
const attemptUnstick = (game, originalPosition) => {
  for (
    let distance = UNSTICK_STEP;
    distance <= UNSTICK_MAX_DISTANCE;
    distance += UNSTICK_STEP
  ) {
...
const findNearestSafeOffset = (
  game,
  originOffset,
  maxRadiusTiles = NEAREST_SAFE_MAX_RADIUS_TILES,
) => {
  ...
  const mapHeight =
    mapWidth > 0 && Array.isArray(game.map[0]) ? game.map[0].length : 0;
...
const ensurePlayerUnstuck = (game) => {
  ...
  const fallback = game.player.lastSafeOffset
    ? { ...game.player.lastSafeOffset }
    : null;
  ...
  if (nearestSafe) {
    ...
    const collisionAfterNearest = runCollisionCheck(
      game,
      "unstick:nearest_result",
    );
  ...
  const safeNearSpawn = findNearestSafeOffset(
    game,
    game.player.offset,
    NEAREST_SAFE_MAX_RADIUS_TILES * 2,
  );
  if (safeNearSpawn) {
...
export const play = (game) => {
  ...
  const inLobby =
    lobby && typeof lobby.isInGame === "function" ? lobby.isInGame() : true;
  if (!inLobby) {
  ...
  if (
    game.player.isCloaked &&
    game.player.cloakExpiresAt &&
    now >= game.player.cloakExpiresAt
  ) {
  ...
  if (game.player.health <= 0) {
    ...
    const elapsedSinceHit = game.player.lastLocalBotDamageAt
      ? now - game.player.lastLocalBotDamageAt
      : Infinity;
    ...
    if (
      awaitingServer &&
      elapsedSinceHit > 700 &&
      !game.player.botDeathConfirmSent
    ) {
  ...
  if (
    ...
    game.audio &&
    game.player &&
    Number.isFinite(game.player.offset?.x) &&
    Number.isFinite(game.player.offset?.y)
  ) {
    ...
    const shouldLoopEngine =
      !isFrozen && !!game.player.isMoving && game.player.health > 0;
    if (shouldLoopEngine && !game.player.engineLoopActive) {
      ...
      (!shouldLoopEngine || game.player.health <= 0) &&
      game.player.engineLoopActive
    ) {
      game.audio.setLoopState(SOUND_IDS.ENGINE, false);
      game.player.engineLoopActive = false;
    }
