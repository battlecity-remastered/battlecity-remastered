...
- Bombs explode 5 seconds after being dropped while armed, wiping nearby items, demolishing server-verified buildings whose footprint falls within the three-tile blast radius (command centers are excluded), and inflicting lethal damage on tanks inside the same radius. Pressing `B` drops an armed bomb immediately (quick deploy), while `D` drops the selected item respecting its current armed state. Bombs placed on the ground do not block player tank movement—tanks can drive over them freely. (client/src/factories/ItemFactory.js:215, client/src/input/input-keyboard.js:128, server/src/hazards/HazardManager.js:414, server/src/BuildingFactory.js:466, client/src/collision/collision-helpers.js:87)
- Active bombs also detonate instantly if their owner dies, ensuring their inventory slot frees up instead of leaving an armed hazard on the field. (server/src/hazards/HazardManager.js:120, server/src/PlayerFactory.js:160)
...
- Research centres must run for 10 seconds at full population (TIMER_RESEARCH) before unlocking their factories; the server streams `research:update` states and rejects early build attempts, while the client renders pending factories as greyed-out menu entries until the timer elapses. (server/src/constants.js:10, server/src/BuildingFactory.js:243, server/src/BuildingFactory.js:640, client/src/factories/BuildingFactory.js:214, client/src/draw/draw-building-interface.js:223)
- Demolish mode stays armed after a successful click so mayors can chain building removals; clicking an empty tile (or right-clicking) cancels the mode and restores the normal cursor. This mirrors the original client’s demolition flow. (client/src/input/input-mouse-core.js:22, original/Battle-City/client/CInput.cpp:487)
- When your city finishes a research line, a toast confirms completion and lists the newly unlocked build options. (client/src/factories/BuildingFactory.js:193)
- Factories require their associated research building to exist with full population to produce items; demolishing or depopulating a research facility immediately stops all dependent factory production. A factory building's required research is determined by type: factory types 100-199 require research types 400-499 (factory type + 300). (server/src/FactoryBuilding.js:29, server/src/BuildingFactory.js:1040)
...
- Bomb icons snap to the tile grid when placed; armed bombs start a 5-second detonation timer, while unarmed bombs remain inert and can be picked up by teammates. Selecting a bomb in inventory does not automatically arm it—you must click the selected bomb icon to toggle its armed state. (client/src/factories/ItemFactory.js:135, client/src/draw/draw-items.js:14, client/src/factories/IconFactory.js:415)
...
- Inventory stacks repeatable items (bombs, mines, turrets) and shows a count overlay; selecting a bomb does **not** automatically arm it. Clicking a selected bomb toggles its armed state, which determines whether pressing `D` drops an armed or unarmed bomb. (client/src/factories/IconFactory.js:415, client/src/draw/draw-panel-interface.js:37)
...
- **Bomb** – Pressing `B` immediately drops an armed bomb (quick deploy shortcut). Pressing `D` drops the currently selected item; if a bomb is selected, it will drop with its current armed state (unarmed by default). To toggle a bomb's armed state, click on it when it's already selected in your inventory. Dropping an armed bomb spawns a synced hazard that later clears items, buildings, and players inside the detonation radius. Bombs placed on the ground can be picked back up by the owning city (press `U` while overlapping) just like mines; enemies cannot collect them. (client/src/input/input-keyboard.js:229, client/src/factories/ItemFactory.js:819, client/src/factories/IconFactory.js:285, client/src/factories/IconFactory.js:334, client/src/factories/IconFactory.js:415, server/src/hazards/HazardManager.js:167)
...
- **Flare Gun** – Holding `CTRL` fires three slow flare shots behind your tank (bullet type `3`) using the classic spread; both client and server honour the lower projectile speed and damage, and each flare now fizzles after roughly one 48px tile so it only tags riders on your tail. (client/src/input/input-keyboard.js:95, client/src/factories/BulletFactory.js:13, server/src/BulletFactory.js:13)
...
- **Turret** – Auto-turrets track targets every 200 ticks and fire standard bullets whenever a hostile tank strays within range. Their shots only damage tanks (including rogues), fizzling on walls or other defenses without chewing through the line; Plasma and Sleeper turrets share the same rule. (client/src/factories/ItemFactory.js:33, client/src/factories/ItemFactory.js:105, client/src/factories/BulletFactory.js:21)
