...
const appendPlayerUpdateIntent = (state: ClientState, nextDirection: number, isMoving: boolean, intents: Intent[]): void => {
    intents.push({
        type: "player.update",
        payload: {
            id: state.local.id ?? "",
...
export const buildTickPlan = (state: ClientState, nowMs: number, dtMs: number): TickPlan => {
    if (!state.local.id) {
        return { intents: [], isMoving: false, direction: state.local.direction, shouldShoot: false };
    }
    ...
    appendPlayerUpdateIntent(state, nextDirection, isMoving, intents);
    ...
    return { intents, isMoving, direction: nextDirection, shouldShoot };
};
